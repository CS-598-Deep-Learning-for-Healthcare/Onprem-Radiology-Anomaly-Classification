...
# keep shuffle=False to match original behavior
...
model = train(1, model, train_dataloader, optimizer, device)
...
def save_metrics(metrics_list, model_name: str, num_epochs: int, lr: float):
    ...
    metrics_path = os.path.join(base_dir, "radbert_metrics.csv")
...
save_metrics([train_metrics, test_metrics], model_name=model_name, num_epochs=1, lr=4e-5)
